...
import { Phone, MapPin, Facebook, X } from "lucide-react";
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-white shadow transition-all duration-300">
        ...
        <div className="container mx-auto px-4 h-full flex items-center justify-between text-sm">
          <div className="flex items-center space-x-4">
            <div className="flex items-center">
              ...
              <span>
                <a href="tel:+233206632799" className=" hover:underline">
                  +233 206632799
                </a>
              </span>
            </div>
      ...
      <nav className="container mx-auto px-4 h-20 flex items-center justify-between">
        <div
          ...
          onClick={() => navigate("/")}
        >
          <img src={logo} alt="Minerals Logo" className="w-14 h-14 mr-2" />
          <div className="flex flex-col items-start">
            <span className="text-xl font-bold">RAMOTH</span>
            <span className="text-sm text-gray-500">SERVICES LIMITED</span>
          </div>
        </div>
...
function NavLink({
  ...
  return (
    <Link
      to={href}
      className={`text-lg uppercase font-medium hover:text-[#0f4de1] transition-colors ${
        isActive ? "text-[#f0730c]" : "text-gray-700"
      }`}
    >
      {children}
    </Link>
